...
const RequestModal = ({visible, onClose, gig, onUpdateSuccess}: GigUpdateModalProps) => {
   ...
   const pickImage = async () => {
    ...
    const sendRequest = async () => {
        ...
        try{
            ...
            if(!budget){
                setBudget(gigDetails.price.toString())
            }
            ...
            await sendRequestToFreelancer(auth.currentUser?.uid || "", gigDetails.userId, gigDetails.id, projectTitle, projectDescription, parseFloat(budget), deadline, attachmentURL)
                    ...
                    <ScrollView bounces={false} showsVerticalScrollIndicator={false}>
                        <View style={{ width:width,alignItems:"center", justifyContent:"space-between"}}>
                            <View className="w-full ">
                                <Pressable onPress={onClose} className=" bg-red-700 p-3 rounded-md items-center w-[10%] top-3 right-3 mb-4 self-end">
                            ...
                            <View className="w-[90%] justify-center mb-5 flex flex-row gap-4 p-3">
                                <Image source={{ uri: gigDetails?.bannerImageUrl }} style={{ width: "50%", height: 100, borderRadius: 8 }} />
                                <View className="w-[45%] justify-center">
                            ...
                            <View className="w-[90%] bg-gray-100 rounded-lg p-4">
                                ...
                                <View className="w-full bg-gray-100 rounded-lg mb-4 items-center gap-4">
                                    <TextInput className="border w-[90%] rounded-lg pl-3" placeholder="Project Title" value={projectTitle} onChangeText={setProjectTitle}></TextInput>
                                    <TextInput className="border w-[90%] h-[200px] rounded-lg pl-3" numberOfLines={10} textAlignVertical="top" placeholder="Project Description" value={projectDescription} onChangeText={setProjectDescription}></TextInput>
                                    <View className="w-[90%] flex-row items-center border rounded-md px-3">
                                        <Text className="absolute left-3 text-gray-500">$</Text>
                                        <TextInput placeholder="Your Budget (Optional)" className="flex-1 p-3 pl-4" keyboardType="numeric" value={budget} onChangeText={setBudget}></TextInput>
                                    </View>
                                    <View className="w-[90%] flex-row items-center justify-between border rounded-md px-3">
                                        <TextInput placeholder="Deadline (Optional)" value={deadline} onChangeText={setDeadline}></TextInput>
                                        <Pressable>
                                    ...
                                    </View>
                                        {
                                            imgAttachment || docAttachment ? (
                                                <View className="w-[90%] flex-row items-center justify-between border rounded-md px-3">
                                                    <TextInput>Attachment Added</TextInput>
                                                ...
                                                <View className="w-[90%] flex-row items-center justify-between border rounded-md px-3">
                                                 <TextInput placeholder="Attachment (Optional)"  ></TextInput>
